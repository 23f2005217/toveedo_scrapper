...
if (fs.existsSync(OUTPUT_FILE)) {
  try {
    ...
    console.log(
      `Loaded ${results.length} previously scraped videos from ${OUTPUT_FILE}`,
    );
  } catch (e) {
...
if (fs.existsSync(VISITED_FILE)) {
  try {
    ...
    console.log(
      `Loaded ${visitedVideos.length} previously visited videos from ${VISITED_FILE}`,
    );
  } catch (e) {
...
// Track episodeUrls already in results for O(1) dedup across parallel workers
const resultEpisodeUrls = new Set(results.map((r) => r.episodeUrl));
    ...
    while (queue.length > 0) {
      ...
      try {
        ...
        const episodeLinks = await page.evaluate(() => {
          return Array.from(
            document.querySelectorAll(".content-item.block.cursor-pointer"),
          ).map((e) => e.href);
        });

        const uniqueEpisodes = [...new Set(episodeLinks)];
        console.log(
          `[Worker ${workerId}] Found ${uniqueEpisodes.length} episodes in playlist.`,
        );
        ...
        for (const epUrl of uniqueEpisodes) {
          ...
          const existing = results.find((r) => r.episodeUrl === epUrl);

          if (visitedVideos.includes(epUrl)) {
            if (existing && existing.videoUrl) {
              console.log(
                `[Worker ${workerId}]   Re-scraping (has m3u8): ${epUrl}`,
              );
            } else {
          ...
          try {
            ...
            if (src) {
              console.log(
                `[Worker ${workerId}]     >>> FOUND: ${title} -> ${src}`,
              );
              const existingIndex = results.findIndex(
                (r) => r.episodeUrl === epUrl,
              );
              if (existingIndex !== -1) {
              ...
              } else if (!resultEpisodeUrls.has(epUrl)) {
                results.push({
                  playlistUrl,
                  episodeUrl: epUrl,
                  title,
            ...
            console.error(
              `[Worker ${workerId}]     Error episode ${epUrl}: ${e.message}`,
            );
          }
        }
      } catch (e) {
        console.error(
          `[Worker ${workerId}] Error playlist ${playlistUrl}: ${e.message}`,
        );
      }
    }
