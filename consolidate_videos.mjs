...
const UPLOADED_VPS = path.join(__dirname, 'uploaded_videos_vps.json');
const DIRECT_REFRESHED = path.join(__dirname, 'direct_videos_refreshed.json');
const MUX_UNMATCHED = path.join(__dirname, 'mux_unmatched_assets.json');
const REMAINING = path.join(__dirname, 'remaining_videos.json');
const OUTPUT = path.join(__dirname, 'consolidated_videos.json');
...
filesToBackup.forEach(file => {
  if (fs.existsSync(file)) {
    fs.copyFileSync(file, path.join(BACKUP_DIR, `${path.basename(file)}.${timestamp}`));
    console.log(`✅ Backed up: ${path.basename(file)} -> backups/${path.basename(file)}.${timestamp}`);
  }
});
...
console.log(`\n📁 Saved to: consolidated_videos.json`);
console.log(`📁 Backups saved to: backups/`);

// Save stats
fs.writeFileSync(path.join(__dirname, 'consolidation_stats.json'), JSON.stringify(stats, null, 2));
